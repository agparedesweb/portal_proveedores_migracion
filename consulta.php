...
if(isset($_POST['buscar'])){
	$desde=$_POST['desde'];
	$hasta=$_POST['hasta'];	
	$query=mysql_query("SELECT reg.folio_interno,reg.folio_uuid, reg.nombre_xml, date(reg.fecha), reg.valido,reg.errores FROM records_xml as reg inner join users as usu on reg.rfc =usu.id WHERE date(reg.fecha) BETWEEN '".$desde."' AND '".$hasta."' AND reg.rfc='".$id."' and reg.cSucursal='".$_SESSION["sucursal"]."'");
