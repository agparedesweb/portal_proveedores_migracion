...
class Multiupload
{
    ...
    */
    public function upFiles($pdfs = array(),$files = array(),$manif = array())
    {
        ...
        $sql_temporada=mysql_query("SELECT CCVE_TEMPORADA FROM CTL_TEMPORADAS WHERE NACTIVO=1");
                        ...
                        {   
                            foreach ($fol as $folio)
                            {
                                ...
                                for ($n=0; $n <= count($mftos)-1; $n++) { 
                                    ...
                                    //$sel=$mftos[$n];
                                    //echo "\n<br>".$sel; 
                                    $sel = substr($orden,$cntmercado, 8);
                                    $mercado = substr($orden,$cntmercado-1, 1);
                                    //echo "<br>Manifs: ".$manifs."<br> y Contador".$cntmercado;
                                    //echo "<br>Manifs: ".$mercado."<br> y Contador".$cntmercado;
                                    $querym=mysql_query("SELECT CFOLIO_MANIF FROM eye_maniffacturados WHERE CRELACIONADO='N' AND CFOLIO_REMIS='".$sel."' AND CMERCADO='".$mercado."' AND CCVE_TEMPORADA='".$temporada."'");
                                    $cfoliom=mysql_result($querym,0);
                                    //ACTUALIZAMOS EL VALOR CRELACIONADO DESPUES DE HABER SELECCIONADO EL MANIFIESTO
                                    //$upd = "UPDATE eye_maniffacturados SET CRELACIONADO='S' WHERE CFOLIO_REMIS='".$sel."' AND CCVE_TEMPORADA='".$temporada."'";
                                    //mysql_query($upd);
                                        ...
                                        if($numRegistros==0) 
                                        {
                                            foreach ($manif as $selectedOption){        
                                                foreach ($selectedOption as $opt){
                                                    $upd = "UPDATE eye_maniffacturados SET CRELACIONADO='S' WHERE CFOLIO_REMIS='".$opt."' AND CCVE_TEMPORADA='".$temporada."'";
                                                    mysql_query($upd); 
                                                }
                                            }  
